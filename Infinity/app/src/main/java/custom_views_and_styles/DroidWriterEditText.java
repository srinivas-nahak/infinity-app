...
public class DroidWriterEditText extends EditText {
    ...
    private void toggleStyle(int style) {
        ...
        if (selectionEnd > selectionStart) {
            ...
            switch (style) {
                ...
                case STYLE_STRIKE:
                    StrikethroughSpan[] strikeSpan = str.getSpans(selectionStart, selectionEnd, StrikethroughSpan.class);

                    // If the selected text-part already has UNDERLINE style on it,
                    // then we need to disable it
                    for (int i = 0; i < strikeSpan.length; i++) {
                        str.removeSpan(strikeSpan[i]);
                        exists = true;
                    }
    ...
    public void onSelectionChanged(int selStart, int selEnd) {
        ...
        if (strikeToggle != null) {
            if (strikedExists)
                strikeToggle.setChecked(true);
            else
                strikeToggle.setChecked(false);
        }
    }
    ...
    public void setStrikeThroughToggleButton(ToggleButton button) {
        strikeToggle = button;

        strikeToggle.setOnClickListener(new Button.OnClickListener() {
            public void onClick(View v) {
                toggleStyle(STYLE_STRIKE);
            }
        });
    }
    ...
    private class DWTextWatcher implements TextWatcher {
        ...
        public void afterTextChanged(Editable editable) {
            ...
            if (position > 0) {
                ...
                StrikethroughSpan currentStrikeSpan=null;
                ...
                if (underlineToggle != null && underlineToggle.isChecked() && currentUnderlineSpan == null) {
                    editable.setSpan(new UnderlineSpan(), position - 1, position, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                ...
                // Handle the striked style toggle button if it's present
                if (strikeToggle != null && strikeToggle.isChecked() && currentStrikeSpan == null) {
                    editable.setSpan(new StrikethroughSpan(), position - 1, position, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                } else if (strikeToggle != null && !strikeToggle.isChecked() && currentStrikeSpan != null) {
                    int strikeStart = editable.getSpanStart(currentStrikeSpan);
                    int strikeEnd = editable.getSpanEnd(currentStrikeSpan);

                    editable.removeSpan(currentStrikeSpan);
                    if (strikeStart <= (position - 1)) {
                        editable.setSpan(new StrikethroughSpan(), strikeStart, position - 1,
                                Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                    }

                    // We need to split the span
                    if (strikeEnd > position) {
                        editable.setSpan(new StrikethroughSpan(), position, strikeEnd,
                                Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                    }
                }
